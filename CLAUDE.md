...
*The daily agent runs entirely via GitHub Actions (remote trigger deleted 2026-08-12). `.github/workflows/daily-briefing.yml` fires hourly (GitHub's scheduler delivers only a few fires/day for this repo, in sweeps); the first fire in the 06:00–11:50 UTC window holds until 11:50 UTC, then runs `scripts/generate_briefing.py` — the briefing lands ~12:00 UTC (~5am PT), just ahead of the 6am PT read deadline. Manual `workflow_dispatch` runs immediately (cancel a holding run in the Actions UI first if one is in flight). The agent's instructions live in the script's `CORE_PROMPT`/`CORE_STRUCTURE` constants, plus the corrections and evidence/format rules at the top of `iran-war-context.md`, loaded every run. Pipeline status and history: `STATE.md`.*
